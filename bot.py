...
BUILD_VERSION = "KIRYSHOP-PRODUCT-PHOTOS-ONLY-2026-08-18"
...
def download_images(urls: list[str], folder: Path, referer: str) -> list[Path]:
    ...
    headers["Accept"] = "image/avif,image/webp,image/apng,image/jpeg,image/*,*/*;q=0.8"
    ...
    for candidate_index, image_url in enumerate(urls, start=1):
        ...
        try:
            ...
            # Most important rule: never publish a size chart/document image.
            if is_size_chart_or_document(source):
                logger.info("Rejected size chart/document: %s", image_url)
                source.unlink(missing_ok=True)
                continue
            ...
            if exact_hash in exact_hashes:
                source.unlink(missing_ok=True)
                continue
            if any(hash_distance(perceptual_hash, existing) <= 4 for existing in perceptual_hashes):
                source.unlink(missing_ok=True)
                continue

            output = folder / f"product_{len(output_files) + 1}.jpg"
    ...
    if not output_files:
        # Deliberately skip the product instead of using a size chart as fallback.
        raise ValueError("Нет безопасных фото товара: размерные сетки/служебные изображения отфильтрованы.")
    return output_files
