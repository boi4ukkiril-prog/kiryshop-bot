...
def was_published(album_id: str) -> bool:
    with db() as connection:
        with connection.cursor() as cursor:
            cursor.execute(
...
def remember_album(album_id: str, album_url: str, category_name: str) -> None:
    with db() as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                ...
                """,
                (album_id, album_url, category_name),
            )
        connection.commit()
...
def category_page(url: str) -> tuple[str, list[str]]:
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if "/albums/" not in full:
            continue
        clean = full.split("?")[0]
        if album_id(clean) and clean not in seen:
            seen.add(clean)
            links.append(clean)
...
def all_album_links(category_url: str, max_pages: int = 100) -> tuple[str, list[str]]:
    ...
    for page in range(1, max_pages + 1):
        ...
        new_links = [link for link in current_links if link not in seen]
        if not new_links:
            break
...
def extract_album(album_url: str) -> tuple[str, list[str]]:
    ...
    for item in candidates:
        ...
        item = "https:" + item if item.startswith("//") else urljoin(album_url, item)

        low = item.lower()
        if not re.search(r"\.(jpg|jpeg|png|webp)(\?|$)", low):
            continue
        if any(x in low for x in ("avatar", "logo", "icon", "qrcode", "favicon")):
            continue
        if item not in seen:
            seen.add(item)
            images.append(item)
    ...
    return title or "New product", images[:MAX_PHOTOS]
...
async def register(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not update.effective_chat or str(update.effective_chat.id) != GROUP_ID:
...
async def publish_album(
    ...
    title, image_urls = await asyncio.to_thread(extract_album, album_url)
    english_title = await asyncio.to_thread(translate_title, title)
