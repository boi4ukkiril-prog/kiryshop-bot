...
def extract_album(album_url: str) -> tuple[str, list[str]]:
    ...
    # Return extra candidates. download_images will keep only 4 truly unique files.
...
def is_size_chart(path: Path) -> bool:
    try:
        with Image.open(path).convert("RGB") as image:
            w, h = image.size
            # Size charts are usually almost white with lots of dark text/lines.
            small = image.resize((64, 64))
            pixels = list(small.getdata())
            bright = sum(1 for r,g,b in pixels if r > 220 and g > 220 and b > 220)
            return bright / len(pixels) > 0.75
    except Exception:
        return False
