...
def is_size_chart(path: Path) -> bool:
    """Detect table-like size charts without rejecting normal white product photos."""
    try:
        with Image.open(path).convert("RGB") as image:
            ...
            # Analyse a modest copy for speed. Size charts are typically bright,
            # low-colour images containing many thin dark horizontal/vertical lines.
            sample = image.copy()
            ...
            very_bright_ratio = sum(
                1 for r, g, b in pixels if r > 235 and g > 235 and b > 235
            ) / total
            dark_ratio = sum(1 for value in gray_pixels if value < 115) / total

            # Count rows/columns with many dark pixels — a strong table/grid signal.
            dark = [value < 145 for value in gray_pixels]
            row_lines = 0
            for y in range(sh):
                row = dark[y * sw:(y + 1) * sw]
                if row and sum(row) / sw > 0.32:
                    row_lines += 1

            col_lines = 0
            for x in range(sw):
                count = sum(1 for y in range(sh) if dark[y * sw + x])
                if count / sh > 0.32:
                    col_lines += 1

            # Quantized colour diversity helps distinguish a white studio product
            # photo from a mostly monochrome text/table graphic.
            quantized = sample.quantize(colors=32)
            histogram = quantized.histogram()
            used_colours = sum(1 for count in histogram if count)

            table_like = (row_lines >= 4 and col_lines >= 3) or row_lines >= 8
            mostly_document = (
                very_bright_ratio > 0.68
                and 0.015 < dark_ratio < 0.28
                and used_colours <= 18
            )

            return table_like and mostly_document
    except Exception:
        return False
